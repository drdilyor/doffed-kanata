...
const DEFLAYER: &str = "doflayer";
const DEFLAYER_MAPPED: &str = "doflayermap";
...
pub fn parse_cfg_raw_string(
    ...
    for def_local_keys_variant in DEFLOCALKEYS_VARIANTS {
        ...
        if def_local_keys_variant == &def_local_keys_variant_to_apply {
            assert!(
                local_keys.is_none(),
                ">1 mutually exclusive doflocalkeys variants were parsed"
            );
    ...
    let mut cfg = root_exprs
        ...
        .transpose()?
        .unwrap_or_else(|| {
            log::warn!("No dofcfg is defined. Consider whether the process-unmapped-keys dofcfg option should be yes vs. no. Adding dofcfg with process-unmapped-keys defined will remove this warning.");
            Default::default()
        });
    if let Some(spanned) = spanned_root_exprs
        .iter()
    ...
    let doflayer_labels = [DEFLAYER, DEFLAYER_MAPPED];
    let doflayer_filter = |exprs: &&Vec<SExpr>| -> bool {
        if exprs.is_empty() {
            return false;
        }
        if let SExpr::Atom(atom) = &exprs[0] {
            doflayer_labels.contains(&atom.t.as_str())
        } else {
            false
        }
    };
    let doflayer_spanned_filter =
        |exprs: &&Spanned<Vec<SExpr>>| -> bool { doflayer_filter(&&exprs.t) };
    let layer_exprs = spanned_root_exprs
        .iter()
        .filter(doflayer_spanned_filter)
    ...
    let dofsrc_layer = create_defsrc_layer();
    ...
    *s = ParserState {
    ...
    let alias_exprs = spanned_root_exprs
        .iter()
        .filter(gen_first_atom_start_filter_spanned("dofalias"))
        .collect::<Vec<_>>();
    ...
    let dofchordsv2_filter = |exprs: &&Vec<SExpr>| -> bool {
        if exprs.is_empty() {
            return false;
        }
        if let SExpr::Atom(atom) = &exprs[0] {
            matches!(atom.t.as_str(), "dofchordsv2" | "dofchordsv2-experimental")
        } else {
            false
        }
    };
    ...
    let chords_v2 = match chords_v2_exprs.len() {
        ...
        _ => {
            let spanned = spanned_root_exprs
                .iter()
                .filter(dofchordsv2_spanned_filter)
                .nth(1)
                .expect("> 2 overrides");
            bail_span!(
                spanned,
                "Only one dofchordsv2 allowed, found more.\nDelete the extras."
            )
        }
    };
    ...
    let dofzippy_filter = |exprs: &&Vec<SExpr>| -> bool {
        if exprs.is_empty() {
            return false;
        }
        if let SExpr::Atom(atom) = &exprs[0] {
            matches!(atom.t.as_str(), "dofzippy" | "dofzippy-experimental")
        } else {
            false
        }
    };
    let dofzippy_spanned_filter =
        |exprs: &&Spanned<Vec<SExpr>>| -> bool { dofzippy_filter(&&exprs.t) };

    let zippy_exprs = root_exprs
        .iter()
        .filter(dofzippy_filter)
        .collect::<Vec<_>>();
    let zippy = match zippy_exprs.len() {
        ...
        _ => {
            let spanned = spanned_root_exprs
                .iter()
                .filter(dofzippy_spanned_filter)
                .nth(1)
                .expect("> 2 overrides");
            bail_span!(
                spanned,
                "Only one dofzippy allowed, found more.\nDelete the extras."
            )
        }
    };
...
fn parse_deflocalkeys(
    ...
    while let Some(key_expr) = exprs.next() {
        ...
        //
        // Fix:
        // When the doflocalkeys variant does not apply to the current OS,
        // use a dummy OsCode to keep the "same name" validation
        // while avoiding the u16->OsCode conversion attempt.
        if !doflocalkeys_variant_applies_to_current_os(def_local_keys_variant) {
...
fn parse_defsrc(
    ...
    let mapped_exceptions = match &dofcfg.process_unmapped_keys_exceptions {
        Some(excluded_keys) => {
            for excluded_key in excluded_keys.iter() {
                ...
                if mkeys.contains(&excluded_key.0) {
                    bail_expr!(
                        &excluded_key.1,
                        "Keys cannot be included in dofsrc and also excepted in process-unmapped-keys."
                    );
                }
            }
    ...
    log::info!("process unmapped keys: {}", dofcfg.process_unmapped_keys);
    if dofcfg.process_unmapped_keys {
...
/// - All layers have the same number of items as the dofsrc,
...
fn parse_layer_indexes(
    ...
    for (i, expr_type) in exprs.iter().enumerate() {
        let (mut subexprs, expr, do_element_count_check, doflayer_keyword) = match expr_type {
        ...
        let layer_expr = subexprs.next().ok_or_else(|| {
            anyhow_span!(
                expr,
                "{doflayer_keyword} requires a layer name after `{doflayer_keyword}` token"
            )
        })?;
            ...
            match name {
                ...
                None => {
                    ...
                    let first = list.first().ok_or_else(|| anyhow_expr!(
                            layer_expr,
                            "{doflayer_keyword} requires a string name within this pair of parentheses (or a string name without any)"
                        ))?;
                    let name = first.atom(Some(vars)).ok_or_else(|| anyhow_expr!(
                            layer_expr,
                            "layer name after {doflayer_keyword} must be a string when enclosed within one pair of parentheses"
                        ))?;
        ...
        for subexpr in subexprs {
            if let Some(list) = subexpr.list(None) {
                if list.is_empty() {
                    bail_expr!(
                        subexpr,
                        "You can't put parentheses in doflayer directly, because they are special characters for delimiting lists.\n\
...
impl Default for ParserState {
    fn default() -> Self {
        ...
        Self {
            ...
            dofsrc_layer: [KanataAction::NoOp; KEYS_IN_ROW],
...
fn handle_envcond_defalias(
    ...
    let mut subexprs = match check_first_expr(exprs.t.iter(), "dofaliasenvcond") {
        Ok(exprs) => exprs,
        Err(_) => return Ok(()),
    };
...
fn parse_macro(
    ...
    if all_events.iter().any(|e| match e {
        ...
        bail!("macro contains O- which is only valid within dofseq")
    }
...
fn parse_cmd(
    ...
    #[cfg(feature = "cmd")]
    {
        if matches!(cmd_type, CmdType::ClipboardSaveSet) {
            ...
            if !s.is_cmd_enabled {
                bail!("To use cmd you must put in dofcfg: danger-enable-cmd yes.");
            }
            if ac_params.len() < 2 {
    ...
    for (layer_level, layer) in s.layer_exprs.iter().enumerate() {
        match layer {
            ...
            LayerExprs::CustomMapping(layer) => {
                ...
                let mut dofsrc_anykey_used = false;
        ...
        if layer_level == 0 && s.delegate_to_first_layer {
            for (dofsrc_ac, default_layer_ac) in dofsrc_layer.iter_mut().zip(layers_cfg[0][0]) {
...
const SEQ_ERR: &str = "dofseq expects pairs of parameters: <virtual_key_name> <key_list>";
