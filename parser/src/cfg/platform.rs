...
pub(crate) fn filter_platform_specific_cfg(
    ...
    doflocalkeys_variant_to_apply: &str,
    lsp_hint_inactive_code: &mut Vec<LspHintInactiveCode>,
) -> Result<Vec<TopLevel>> {
    let valid_platform_names = DEFLOCALKEYS_VARIANTS
        .iter()
        .map(|dfl| dfl.trim_start_matches("doflocalkeys-"))
        .collect::<Vec<_>>();
    let current_platform = doflocalkeys_variant_to_apply.trim_start_matches("doflocalkeys-");
    top_levels
        .into_iter()
