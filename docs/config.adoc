...
Your configuration file must have exactly one `dofsrc` list.
This defines the order of keys that the `+doflayer+` entries will operate on.

.Syntax:
[source]
----
(dofsrc $key1 $key2 ... $keyN)
----

[cols="1,6"]
|===
| `$key`
| The name of a key. This can be a default key name or one defined in <<doflocalkeys>>.
...
The `dofsrc` configuration entry defines which of your key inputs
will be processed by kanata and how the keys map to defined layers.
Keys excluded from `dofsrc` will not be processed by Kanata
unless you have `process-unmapped-keys yes` in <<introduction-dofcfg,dofcfg>>.
...
An example `dofsrc` containing the US QWERTY keyboard keys as an
...
Your configuration file must have at least one `+doflayer+` entry. This defines
how each physical key mapped in `+dofsrc+` behaves when kanata runs.

.Syntax:
[source]
----
(doflayer $layer-name $action1 $action2 ... $actionN)
----
...
when the corresponding `dofsrc` input key is pressed.
|===

**Description**

A `+doflayer+` configuration entry is followed by the layer name then a list of
keys or actions. The usable key names are the same as in dofsrc. Actions are
explained further on in this document. The whitespace story is the same as with
`+dofsrc+`. The order of keys/actions in `+doflayer+` corresponds to the
physical key in the same sequence position defined in `+dofsrc+`.
...
An example `dofsrc` and `doflayer` that remaps QWERTY to the Dvorak layout
would be:
...
layer icons in `+doflayer+` and `+doflayermap+` to show in the tray menu on layer activation,
...
An alternative method for defining a layer exists: `doflayermap`.
This method maps inputs to actions by defining input-output pairs,
ignoring `dofsrc` entirely.

You will likely want to either enable <<process-unmapped-keys>>
or define most of your keyboard keys within <<dofsrc>> when using `doflayermap`.
...
The `doflayermap` variant has the advantage of terser configuration
when only a few keys on a layer need to be mapped.
When practicing a new configuration, the standard `doflayer` has an advantage
of looking more like a physical keyboard layout,
which may be helpful to some.

Within `doflayermap`, the very first item must be the layer name.
The layer name must be in parentheses unlike with `doflayer`.
...
[source]
----
;; dofsrc is still necessary
(dofsrc)
(doflayermap (base-layer)
  caps esc)
----

The input key takes the same role as `dofsrc` keys.
The output action takes the role that items in the normal `doflayer` have.
...
|===
| `&#95;`
| Map all unmapped keys in this layer that are defined in `dofsrc`.

| `&#95;&#95;`
| Map all unmapped keys in this layer that are not defined in `dofsrc`.
...
because they may have implications on what you should include/exclude in `dofsrc`.
The Windows LLHOOK I/O mechanism has the most issues by far.

[[key-names]]
== Key names for dofsrc and doflayermap
...
that are not allowed in `dofsrc` by default, at least according to the symbol shown
...
you may want to use `doflayermap` if remapping using these key names.
...
You can use `doflocalkeys` to define additional key names that can be
used in `dofsrc`, `doflayer`, and anywhere else in the configuration.

.Syntax:
[source]
----
(doflocalkeys-$variant
...
Only one of each doflocalkeys-* variant is allowed. The variants that are not
applicable will be ignored, e.g. `doflocalkeys-linux` and `doflocalkeys-wintercept`
are both ignored when using the default Windows `kanata.exe` binary.

**Description**

The `doflocalkeys` configurations are not strictly necessary.
...
There are five variants of doflocalkeys:
...
[[introduction-dofcfg]]
== Introduction to dofcfg

Your configuration file may include a single `dofcfg` entry.
The `dofcfg` can be empty or omitted.
...
All other options can be found later in the <<optional-dofcfg-options>> section.

.Example of an empty dofcfg:
[source]
----
(dofcfg)
----
...
The `process-unmapped-keys` option in `dofcfg` is probably the most
generally impactful option.
Enabling this configuration makes kanata process keys
that are not defined in `dofsrc`.
This might be useful
if you are only mapping a few keys in dofsrc
instead of most of the keys on your keyboard.

By default, keys excluded from `dofsrc` will not work in various scenarios.
Some examples:
...
(dofcfg process-unmapped-keys (all-except lctl ralt))
----
...
Using the `dofalias` configuration entry, you can introduce a shortcut label
for an action.
...
The `dofalias` entry reads pairs of items in a sequence
...
`dofalias` is where we'll first see nested lists in this guide.
...
This alias can be used in `doflayer` as a substitute for the long action. The
...
You may have multiple `dofalias` entries and multiple aliases within a single
`dofalias`. Aliases may also refer to other aliases that were defined earlier
in the configuration file.

.Example:
[source]
----
(dofalias one (tap-hold 200 200 caps lctl))
(dofalias two (tap-hold 200 200 esc lctl))
(dofalias
...
You can choose to put actions without aliasing them right into `doflayer`.
However, for long actions it is recommended not to do so to keep a nice visual
alignment. Visually aligning your `doflayer` entries will hopefully make your
...
----
(doflayer example
  ;; this is equivalent to the previous doflayer example
...
Using the `dofvar` configuration entry,
...
Similar to how `dofalias` works,
`dofvar` reads pairs of items in a sequence
...
- variables cannot be used in `dofcfg`, `dofsrc`, or `doflocalkeys`
...
Within the second item of `dofvar`,
...
the active kanata layer will be the first `doflayer` defined in the configuration.

.Example:
[source]
----
(doflayer has-live-reload
  lrld a s d f
)
----
...
defined in a `doflayer` entry.

.Example:
[source]
----
(dofalias dvk (layer-switch dvorak))
----
...
[source]
----
(dofalias nav (layer-while-held navigation))
----
...
* if `delegate-to-first-layer` is enabled: the first layer defined by `doflayer` or `doflayermap`
* `dofsrc`
...
NOTE: If `delegate-to-first-layer` is enabled and the base layer is currently set to the first defined layer, a transparent key on the base layer will fall through directly to `dofsrc`.
...
|===
| `use-dofsrc`
| String action that outputs the corresponding `dofsrc` input key.
|===

**Description**

A similar concept to transparent key is the `+use-dofsrc+` action.
When activated, the underlying `dofsrc` key will be the output action.

.Example:
[source]
----
(dofsrc a b c d)
(dofalias src use-dofsrc)
(doflayer remap-only-c-to-d
  _ _ d @src)
----
...
<<linux-only-linux-unicode-u-code,unicode behaviour customization>> in dofcfg.
...
* `release-key` or `key↑`: release a key, accepts `dofsrc` compatible names
...
(doflayer alted-with-exceptions
  _    _    _    @lft @rgt
)
----
...
the hold actions can be used within `dofsrc` and `doflayermap`
...
changing dofsrc then live-reloading will not
begin handling mouse events
if dofsrc previously did not have any mouse events in dofsrc.
...
the above actions are also usable in `dofsrc`
...
You may use these key names within `dofsrc`
...
When doing so, using the `mwu`, `mwd`, `mwl`, `mwr` key names in `dofsrc`
...
There is a toggable dofcfg option related to `movemouse-accel` - <<movemouse-inherit-accel-state>>. You might want to enable it, especially if you're coming from QMK.
...
NOTE: When using one-shot with keys that will trigger dofoverrides,
you will likely want to adjust <<override-release-on-activation>> to yes in `dofcfg`.
...
With `(dofvar $tap-repress-timeout 0)`, the output event sequence is:
...
or in other words the key that corresponds to `dofsrc`.
This is in contrast to the `fork` and `switch` actions
which operates on outputted keys, or in other words the outputs
that are in `doflayer`, `dofalias`, etc. for the corresponding `dofsrc` key.
...
NOTE: In case the modifiers to be undone are not part of `dofsrc`,
<<process-unmapped-keys>> needs to be enabled in `dofcfg` in order for their
...
If using `real`, this will check against the dofsrc inputs.
...
[source]
----
(dofalias switch-input-history-example
  (switch
...
Additionally you must add the <<danger-enable-cmd>> `dofcfg` option.
...
which you may desire to do for reuse via `dofvar`.
...
- **No special flags**: Does not require `danger-enable-cmd` in dofcfg
...
[source]
----
(dofalias ab1 (arbitrary-code 700))
----

[[global-overrides]]
== Global overrides

The `dofoverrides` optional configuration item allows you to create global
...
Only zero or one `dofoverrides` is allowed in a configuration file.

NOTE: Depending on your use case
you may want to adjust <<override-release-on-activation>> in `dofcfg`.
...
(platform (win winiov2 wintercept)
  (dofalias run-my-script (cmd #| something involving powershell |#))
)

(platform (macos linux)
  (dofalias run-my-script (cmd #| something involving bash |#))
)
----
...
You may define a single `+dofchordsv2+` configuration item.
...
The `+dofchordsv2+` feature is configured as shown below:
...
|===
| `$participating-keys`
| These are key names you would use in `dofsrc`.
A minimum of two keys must be defined per chord.
The list must be unique per chord.

| `$action`
| These are actions as you would configure in `doflayer` or `dofalias`.
...
Input chords have a related `dofcfg` item: <<chords-v2-min-idle>>.
...
so that the `dofcfg` item does not need to be adjusted back and forth
...
will delegate to the corresponding dofsrc key
...
other than those that exist in `dofsrc` a no-op.
...
This configuration item changes activation behaviour from `dofoverrides`.
...
(doffakekeys B (layer-while-held block))
...
|===
| `keyboard-mice`
| When no mouse events are in `dofsrc`.

| `any`
| When any mouse buttons or mouse scroll events are in `dofsrc`.
|===
...
if you're using `process-unmapped-keys yes` or have the key in your dofsrc.
...
**and** also omit both `ralt` and `lctl` from `dofsrc`.
...
NOTE: Even with these workarounds, putting `+lctl+` and `+ralt+` in your dofsrc may not
...
This dofcfg item allows you to intercept mouse buttons for a specific mouse device.
...
dofcfg item defined with any numbers. Then when a button is first pressed on
the mouse device, kanata will print its hwid in the log; you can then
copy-paste that into this configuration entry. If this dofcfg item is not
...
This dofcfg item allows you to intercept only specific keyboards.
...
you can run the variant with this dofcfg item empty.
...
If this dofcfg item is not defined, the log will not print.
...
This dofcfg item allows you to exclude certain keyboards from being intercepted.
...
This dofcfg item allows you to exclude certain mice from being intercepted.
...
Don't show a tooltip layer icon for the base layer (1st doflayer). Defaults to true. Requires <<windows-only-win-tray>> gui-enabled build.
...
[[using-multiple-dofcfg-options]]
=== Using multiple dofcfg options

The `dofcfg` entry is treated as a list with pairs of strings. For example:
...
An example dofcfg containing many of the options is shown below. It should be
...
Your final desired `dofcfg` could be to have logging of layer changes as false.
...
Configuring this entry is similar to `+dofalias+`,
...
Fake keys can be defined inside of `doffakekeys`.
...
[source]
----
(dofseq git-status (g s t))
(defvirtualkeys git-status (macro g i t spc s t a t u s))
(dofalias rcl (tap-hold-release 200 200 sldr rctl))
...
(doflayer base
        sldr nop0 nop1 nop2)
(doftemplate seq (vk-name input-keys output-action)
    (defvirtualkeys $vk-name $output-action)
    (dofseq $vk-name $input-keys)
)
...
Within the key list of `dofseq` configuration items,
...
(doftemplate seq (vk-name in out)
	(defvirtualkeys $vk-name $out)
	(dofseq $vk-name $in))

(defvirtualkeys rls-sft (multi (release-key lsft)(release-key rsft)))
(dofvar rls-sft (on-press tap-vkey rls-sft))
(doftemplate rls-sft () $rls-sft 5)
...
(dofalias dot-sequence (macro (sequence 250) 10 .))

;; Enter sequence mode and input . with a timeout of 250 and using hidden-delay-type
(dofalias dot-sequence (macro (sequence 250 hidden-delay-type) 10 .))
----
...
Input chords are configured similarly to `+dofalias+` with two extra parameters
at the beginning of each `+dofchords+` group: the name of the group and a
...
Note that unlike with `+dofseq+`, these keys do not directly correspond to real
...
There is a variant of `dofalias`: `dofaliasenvcond`.
...
A use case for `dofaliasenvcond` is when one has multiple devices
...
[source]
----
(dofaliasenvcond (LAPTOP lp1)
  met @lp1met
)

(dofaliasenvcond (LAPTOP lp2)
  met @lp2met
)
----
...
The top-level configuration item `doftemplate`
...
The parameters to `doftemplate` in order are:
...
One consequence of this early parsing is that variables defined in `dofvar`
...
[source]
----
(doftemplate alt-fork (original-action new-action)
  (fork $original-action (multi (unmod (ralt lalt) nop0) $new-action) (lalt ralt))
)
(dofsrc 1 2 3)
(dofalias fn1 (template-expand alt-fork 1 f1))
;; Templates are a simple text substitution, so the above is exactly equivalent to:
;; (dofalias fn1 (fork 1 (multi (unmod (ralt lalt) nop0) f1) (lalt ralt)))
(dofalias fn2 (template-expand alt-fork 2 f2))
;; You can use t! as a short form of template-expand
(dofalias fn3 (t! alt-fork 3 f3))
(doflayer default @fn1 @fn2 @fn3)
----
...
.Example 5:
----
;; dofvar is parsed AFTER template expansion occurs.
(dofvar a hello)

(doftemplate template1 (var1)
...
;; with its dofvar value of: hello.
(template-expand template2 $a)
----

[[concat-in-doftemplate]]
==== concat in doftemplate

Like <<concat-in-dofvar,concat in dofvar>>,
a list beginning with `concat` within the content of `doftemplate`
...
You may define a single `+dofzippy+` configuration item.
...
This must be the first item following `dofzippy`.
...
as one would configure in `dofsrc`.
