...
that are not allowed in `dofsrc` by default, at least according to the symbol
shown. You can use `doflocalkeys` to define additional key names that can be
used in `dofsrc`, `doflayer` and anywhere else in the configuration.

There are four variants of doflocalkeys:
...
Enabling this configuration makes kanata process keys that are not in dofsrc.
This is useful if you are only mapping a few keys in dofsrc instead of most of
...
This dofcfg item allows you to intercept mouse buttons for a specific mouse
...
Similar to how `dofcfg` works, `dofalias` reads pairs of items in a sequence
where the first item in the pair is the alias name and the second item is the
action it can be substituted for. However, unlike `+dofcfg+`, the second item
in `dofalias` may be a "list" as opposed to a single string like it was in
`dofcfg`.
...
* `release-key`: release a key, accepts `dofsrc` compatible names
...
A fake key can be defined in a `+doffakekeys+` configuration entry. Configuring
this entry is similar to `+dofalias+`, but you cannot make use of aliases
inside of `+doffakekeys+` to shorten an action. You can however refer to
...
Sequences are configured similarly to `+doffakekeys+`. The first parameter of a
...
(doffakekeys git-status (macro g i t spc s t a t u s))
