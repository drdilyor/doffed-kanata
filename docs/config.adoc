...
Your configuration file must have exactly one `dofsrc` entry. This defines the
order of keys that the `+doflayer+` entries will operate on.

A `dofsrc` entry is composed of `dofsrc` followed by key names that are
separated by whitespace.

It should be noted that the `dofsrc` entry is treated as a long sequence; the
amount of whitespace (spaces, tabs, newlines) are not relevant. You may use
spaces, tabs, or newlines however you like to visually format `dofsrc` to your
liking.
...
instead of relying on matching the `dofsrc` ordering.
...
`&#95;` maps only keys that are in dofsrc.

`&#95;&#95;` excludes mapping keys that are in dofsrc.
...
or map most of your keyboard keys within dofsrc when using `doflayermap`.
...
will not work correctly for subsequent key presses that are not in dofsrc.
...
You may define a single `+dofchordsv2-experimental+` configuration item.
...
The `+dofchordsv2-experimental+` feature is configured as shown below:
...
Input chords have a related `dofcfg` item: <<chords-v2-min-idle-experimental>>.
...
(ralt) if you're using that key in your dofsrc. This is applicable for many
...
NOTE: Even with these workarounds, putting `+lctl+`+`+ralt+` in your dofsrc may not
...
;; (dofalias fn1 (fork 1 (multi (unmod ralt lalt) f1) (lalt ralt)))
