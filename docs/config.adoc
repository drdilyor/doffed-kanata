...
key in a `+doflayer+`. The behaviour depends if `+_+` is on a base layer or a
while-held layer. When `+_+` is pressed on the active base layer, the key will
default to the corresponding `dofsrc` key. If `+_+` is pressed on the active
...
With `(dofvar $tap-timeout 0)`, the output event sequence is:
...
(doftemplate seq (vk-name input-keys output-action)
        ...
        sldr(dofseq $vk-name $input-keys)
)
...
You may define a single `+dofzippy-experimental+` configuration item.
...
This must be the first item following `dofzippy-experimental`.
