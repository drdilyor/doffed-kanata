...
fn special_nop_keys() {
    let result = simulate(
        "(dofcfg sequence-input-mode visible-backspaced)
         (dofsrc a b c d e)
         (doflayer base sldr nop0 c nop9 0)
         (defvirtualkeys s1 (macro h i))
         (dofseq s1 (nop0 c nop9))
        ",
...
const OVERLAP_CFG: &str = "
    ...
    (dofseq s3 (O-(c d) e))
    (dofseq s4 (c d e))
    (dofseq s5 (O-(c d) O-(f g)))
    (dofseq s6 (O-(c d) f g))
    (dofseq s7 (c d O-(f g)))
    ;; (dofseq s8 (c d f g)) KNOWN BUGGY CASE! breaks s6 detection
    ";
...
fn chorded_keys_hidden_delaytype() {
    let result = simulate(
        "(dofcfg sequence-input-mode hidden-delay-type)
         (dofsrc 0)
         (doflayer base sldr)
         (defvirtualkeys s1 z)
         (dofseq s1 (S-(a b)))
