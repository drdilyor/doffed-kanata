...
fn on_press_delay_must_be_single_threaded() {
    ...
    let result = simulate(
        "(dofsrc) (doflayermap (base) a (on-press-delay 10))",
        "d:a t:50 u:a t:50",
    );
    assert_eq!("", result);
...
fn on_release_delay_must_be_single_threaded() {
    let start = std::time::Instant::now();
    let result = simulate(
        "(dofsrc) (doflayermap (base) a (on-release-delay 10))",
        "d:a t:50 u:a t:50",
    );
    assert_eq!("", result);
    let end = std::time::Instant::now();
...
fn no_delay_must_be_single_threaded() {
    let start = std::time::Instant::now();
    let result = simulate("(dofsrc) (doflayermap (base) a XX)", "d:a t:50 u:a t:50");
    assert_eq!("", result);
    let end = std::time::Instant::now();
    let duration = end - start;
